...
from typing import (
    ContextManager, Dict, Hashable, Iterable,
...
class TestJATICObjectDetector:
    ...
    @pytest.mark.parametrize("detector, id2name, img_batch_size, expectation", [
        (MagicMock(spec=od.Model), dummy_id2name, 2,
    ...
    @pytest.mark.parametrize(
        "detector_output, id2name, img_batch_size, imgs, expected_return",
        [
            ([dummy_out], dummy_id2name, 2, [np.random.randint(0, 255, (256, 256, 3), dtype=np.uint8)], dummy_expected),
            ([dummy_out], dummy_id2name, 1, [np.random.randint(0, 255, (256, 256), dtype=np.uint8)], dummy_expected),
            ([dummy_out] * 2, dummy_id2name, 2, np.random.randint(0, 255, (2, 256, 256), dtype=np.uint8),
                [dummy_expected[0]] * 2),
            ([MagicMock(spec=od.ObjectDetectionTarget, boxes=[], labels=[], scores=[])], dummy_id2name, 1,
