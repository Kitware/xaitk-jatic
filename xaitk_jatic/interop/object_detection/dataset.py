...
if not is_usable:
    LOG.warning("COCOJATICObjectDetectionDataset requires additional dependencies, please install 'xaitk-jatic[tools]'")
else:
    class COCOJATICObjectDetectionDataset(Dataset):
        ...
        def __init__(
            ...
            for img_idx, img_id in enumerate(kwcoco_dataset.imgs.keys()):
                bboxes = np.empty((0, 4))
                labels = []
                scores = []
        ...
        def __getitem__(self, index: int) -> OBJ_DETECTION_DATUM_T:
            ...
            self._image_metadata[image_id].update(dict(
                id=image_id,
                image_info=dict(
                    width=width,
                    height=height,
